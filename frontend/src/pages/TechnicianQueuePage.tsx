...
import { Chip, List, ListItemButton, ListItemText, MenuItem, TextField, Typography } from "@mui/material";
...
export default function TechnicianQueuePage() {
  ...
  return (
    <>
      ...
      {query.isError && (
        <ErrorState
          message={query.error instanceof ApiError ? query.error.message : "Erro ao carregar a fila."}
          onRetry={() => query.refetch()}
        />
      )}
      {query.data && query.data.length === 0 && <EmptyState message="Nenhum chamado nesta fila." />}
        ...
        {query.data?.map((ticket) => (
          <ListItemButton key={ticket.id} component={Link} to={`/chamados/${ticket.id}`}>
            <ListItemText
              ...
              secondary={ticket.assignee ? `Responsavel: ${ticket.assignee.name}` : "Sem responsavel"}
            />
