...
import type { DashboardEmployeeSummary, DashboardTechnicianSummary } from "../types/domain";
...
export default function DashboardPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      apiFetch<DashboardEmployeeSummary | DashboardTechnicianSummary>("/api/v1/dashboard/summary"),
  });

  if (query.isLoading) return <LoadingState label="Carregando painel..." />;
  if (query.isError) {
    const message = query.error instanceof ApiError ? query.error.message : "Erro ao carregar o painel.";
    ...
    <>
      ...
      {!isTechnician && "account_locked" in data && (
        <Grid container spacing={2} sx={{ mb: 2 }}>
          <MetricCard label="Conta bloqueada" value={data.account_locked ? "Sim" : "Nao"} />
      ...
      {isTechnician && "unassigned_tickets" in data && (
        <Grid container spacing={2}>
          ...
          {Object.entries(data.by_status).map(([status, count]) => (
            <MetricCard key={status} label={`Status: ${status}`} value={count} />
          ))}
          {Object.entries(data.by_priority).map(([priority, count]) => (
            <MetricCard key={priority} label={`Prioridade: ${priority}`} value={count} />
          ))}
        </Grid>
      )}
    </>
